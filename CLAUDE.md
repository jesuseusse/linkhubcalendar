...
**Users list features:** consolidated identity cell (32px photo/initial-avatar + name + `@username` + email stacked in one column; name links to the public profile in a new tab when `username` is set); plan + Stripe status cell (plan pill plus a colored status line — "Activo"/"Pago pendiente"/"Cancela al final del periodo" — reusing the same wording/logic as `BillingClient.tsx`); links count; **WhatsApp** — scans `user.links[]` for the first link `getSocialIcon()` (from `SocialIcons.tsx`) tags as WhatsApp, then extracts a raw digit string via a local `extractWhatsappNumber()` in `UserTable.tsx` (handles both `wa.me/<digits>` path-based links and `api.whatsapp.com/send?phone=<digits>` query-param links) and renders it as a `wa.me/<digits>` link with the brand icon — `—` if the user has no WhatsApp link; **Citas (30d)** — appointments booked in the trailing 30 days, aggregated server-side (see below); "Última actividad" as relative time (`Intl.RelativeTimeFormat`, es-MX) and "Creada" as an absolute date; `createdAt`/`updatedAt` columns remain clickable for client-side sort (asc/desc); server-side cursor pagination (100 users per batch via `/api/super-admin/users/paginated`, "Cargar más" button appends next batch); server-side plan filter dropdown (free/pro/team/all — `plan` query param, requires Firestore composite index `plan ASC + createdAt DESC` on `users` collection group, defined in `firestore.indexes.json`); "Exportar correos" modal with comma-separated email list paginated 1000 at a time with a clipboard copy button. The table itself is a bounded scroll box (`max-h-150 overflow-auto`) with a `sticky` `<thead>` — only the table scrolls on either axis when its content overflows, never the page.

**Appointments-30d aggregate & caching:** `IAppointmentRepository.countInDateRange(tenantId, userId, fromDate, toDate)` runs a Firestore `.count()` query (`type == 'appointment' && date` in range) per user, wrapped in `unstable_cache` with `revalidate: 86400` (24h) and tag `CacheTags.appointmentsCount(tenantId, userId)` — implemented in `FirestoreAppointmentRepository.ts`. `GetUsersPaginatedUseCase` (the use case backing the paginated endpoint) fans this out via `Promise.all` across the page's users and attaches the result as `appointmentsLast30d` on `UserSummaryDto`. `GetAllUsersUseCase` (the legacy, unpaginated `/api/super-admin/users` route — dead code, no UI caller) skips the aggregate and always returns `0` to avoid an unbounded batch of count queries.

**Metrics deliberately not included (no backing data yet):** profile views and link clicks have zero instrumentation anywhere in the app — the public profile page and link anchors fire no tracking events, and the campaign email click-tracker (`src/lib/campaign/trackingUrl.ts`) is unrelated/not reusable for this. Adding these requires new event capture + storage + aggregation, out of scope for the table redesign.
