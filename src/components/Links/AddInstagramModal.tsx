...
export function AddInstagramModal({ onConfirm, onClose, loading }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    setError(null);
    try {
      await onConfirm({ title: 'Instagram', url: `https://instagram.com/${cleanUsername}` });
    } catch (err: unknown) {
    ...
    <div className='fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4'>
      <div className='bg-surface w-full max-w-sm p-6 relative rounded'>
        ...
        <div className='flex items-center gap-2 mb-1'>
          <InstagramIcon className='w-5 h-5 fill-[#E1306C] shrink-0' />
          <h2 className='text-sm font-semibold text-foreground'>Agregar Instagram</h2>
        </div>
