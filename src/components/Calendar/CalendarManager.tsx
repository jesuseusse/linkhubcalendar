				...
				<div className='flex-1'>
					{selectedDate ? (
						<div>
							...
							<form onSubmit={handleAddSlot} className='flex gap-2 mb-4'>
								<input
									...
									className='px-2 py-1 text-sm border border-zinc-300 focus:outline-none focus:border-zinc-900'
								/>
								<input
									type='time'
									value={endTime}
								...
								<button
									type='submit'
									disabled={loading}
									className='px-3 py-1 text-xs font-medium bg-zinc-900 text-white hover:bg-zinc-800 disabled:opacity-50 transition-colors'
								>
							...
							{slotsForDate.length > 0 ? (
								<ul className='space-y-2'>
									{slotsForDate.map(slot => (
										<li
											key={slot.id}
											className='flex items-center justify-between bg-zinc-50 border border-zinc-200 px-3 py-2'
										>
											...
											<div className='flex gap-1'>
												{slot.booked ? (
													<>
														<span className='text-xs text-amber-600 px-2 py-0.5 bg-amber-50 border border-amber-200'>
															Reservado
														</span>
														<button
						...
						<div className='py-8 text-center'>
							<p className='text-sm text-zinc-400'>Selecciona una fecha</p>
						</div>
					)}
				</div>
			</div>
