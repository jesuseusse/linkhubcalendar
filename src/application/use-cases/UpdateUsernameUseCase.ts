...
export class UpdateUsernameUseCase {
  ...
  async execute(tenantId: string, userId: string, username: string): Promise<UserResponseDto> {
    ...
    const existing = await this.userRepository.findByUsername(tenantId, username);
    if (existing && existing.id !== userId) {
      throw new Error("Username is already taken");
    }

    const updated = await this.userRepository.updateUsername(tenantId, userId, username);
    if (!updated) {
      throw new Error("User not found");
    }
